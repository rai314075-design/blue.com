...
var myTuple: [number, string, boolean] = [1, "hello", true];
...
const myLiteral: "hello" = "hello";
// myLiteral = "world"; // Error: Only "hello" allowed
let myLiteralUnion: "hello" | "world";
...
let arr: [number, string, boolean] = [12, "rishi", true];
...
let firstVariable: string | null = null;
let secondVariable: number | null = null;

// 'undefined': Default for uninitialized variables.
let undefinedVariable: string; // undefined
let secondUndefinedVariable: number | undefined; // undefined
...
let annotatedVariable: number = 10;
...
// Interfaces: Define object shapes with properties and types. Type Aliases: Create names for complex types.
...
//greet({ name: "Alice", age: 30, email: "alice@example.com", gender: "Female" });

// Interface extension: Inherits properties from another interface.
interface User {
  name: string;
  email: string;
  password: string;
...
let ten: string | number; //this variable can hold either a string or a number value, but not both at the same time.the line in the center is called union type because it allows for multiple types to be assigned to a single variable. In this case, the variable 'b' can hold either a string or a number, but not both at the same time. This provides flexibility while still maintaining type safety, as TypeScript will enforce that only the specified types can be assigned to the variable.
ten = "hello"; // This is valid because 'b' can hold a string value.

//what is intersection types
// Intersection types in TypeScript allow you to combine multiple types into a single type that has all the properties and methods of the combined types. An intersection type is created using the ampersand (&) symbol to separate the different types that you want to combine. This is useful when you want to create a new type that includes all the features of multiple existing types. In the example below, we have defined two interfaces, 'A' and 'B', and then created an intersection type called 'C' that combines both interfaces. This means that any variable of type 'C' must have all the properties and methods defined in both 'A' and 'B'.

type ofUser = { name: string };

type ofAdmin = ofUser & {
  getDetails(user: string): void;
};

function five(a: ofAdmin) {
...
class Employee {
  constructor(
    ...
    public name: string, // This creates a public property 'name' and initializes it with the value passed to the constructor
    private id: number, // This creates a private property 'id' and initializes it with the value passed to the constructor
    protected department?: string, // This creates an optional protected property 'department' and initializes it with the value passed to the constructor
  ) {
...
function adbde(name: string, age: number, cb: (arg: string) => void) {
  cb("rishi");
}

adbde("rajveer", 21, (arg: string) => {
...
function hi(name: string, gender: string = "not to be disclosed") {
...
let spread = [1, 2, 3] as const; //like this or like after this line i gave the second example for this
console.log(sum(...spread)); // Output: 6

//let spread = [number,number,number] = [1, 2, 3]; // This is another way to define the 'spread' variable with a specific type annotation for an array of three numbers.
// console.log(sum(...spread)); // Output: 6

//What is function overloads in typescript
// Function overloads in TypeScript allow you to define multiple function signatures like same function name again and again for a single function implementation.
// his means you can specify different parameter types and return types for the same function name, providing flexibility in how the function can be called.
// The actual implementation of the function must be able to handle all the defined overloads, and TypeScript will determine which overload to use based on the arguments passed when the function is called.

// Example of function overloads in TypeScript:

// 1. Overload signatures (No changes needed here)
function six(a: string): void;
function six(a: string, b: string): string;

// 2. Implementation signature
// Change: The return type must accommodate both 'void' and 'string'
function six(a: string, b?: string): string | void {
...
identity<string>("Hello, TypeScript!"); // Output: "Hello, TypeScript!"
identity<number>(42); // Output: 42

//example of generic interface in typescript
...
function abcdef(obj: Box<string>) {
...
class Boxs<T> {
  ...
  constructor(
    ...
    public name: string,
    public gender: string, // The 'gender' parameter has a default value of "male", which means that if no value is provided for 'gender' when creating an instance of the 'Boxs' class, it will automatically be set to "  male".
    public isstudent: boolean,
    public age: number | null,
  ) {}
}

class Container<T> extends Boxs<T> {
...
let stringContainer = new Container<string>( // This creates an instance of the 'Container' class with the type parameter 'T' set to 'string'. The constructor of the 'Container' class is called with specific arguments to initialize the properties of the instance.
  "This is a name from  Container",
  "This is a container value!<T>",
  89,
);
...
let c1 = new bottle<string>("hey");
let c2 = new bottle<number>(123);
...
ytug<string>("hey", "hello");


//what is exporting and importing modules

import { gerDetails ,addPayment} from "./payment";
...
import BottleSeller from "./payment";

let Payments = new BottleSeller("rishi",758);
let PaymesntOne = new BottleSeller("rajveer",78);
...
(typeAssertion as number).toFixed(2)//example of type assertion
...
let nonNUll : null | undefined | string;
...
function guard (agr : string | number|any){
  ...
  else {throw new Error("Pagal how gaya hai kya fraaaaands");
...
function switchoffKaro(device: tykaRemote | carkaRemote){
